...
void avlPurgeAvlTree (avlTree_t *avl_tree, unsigned int max_entries)
{
  ...
  avl_tree->root.link[_LEFT] = 0;
  avl_tree->root.link[_RIGHT] = 0; 
  avl_tree->count = 0;
  avl_tree->value = 0;
  ...
  {
    treeHeap[i].link[_LEFT] = 0;
    treeHeap[i].link[_RIGHT] = &treeHeap[i+1];
