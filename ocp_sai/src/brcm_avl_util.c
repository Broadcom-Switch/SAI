...
void * avlRemoveEntry( avlTree_t *avl_tree, void *item)
{
   unsigned int done = 0;
   int depth;
   char pathMap[_AVL_MAX_DEPTH+3];
   avlTreeTables_t *path[_AVL_MAX_DEPTH+3];               
   avlTreeTables_t **nodePtr;
   avlTreeTables_t *node = avl_tree->root.link[_LEFT];
  
   if (node == NULL)
     return NULL;

   pathMap[_LEFT] = _LEFT;
   path[_LEFT] = &avl_tree->root;

   /* Find item to remove */
   for ( depth = 1; depth < _AVL_MAX_DEPTH; depth++)
   {
     ...
     {
       node = node->link[_RIGHT];
       pathMap[depth] = _RIGHT;
     }
     else if (diff == AVL_LESS_THAN)
     {
       node = node->link[_LEFT];
       pathMap[depth] = _LEFT;
     }
     if (node == NULL)
       return NULL;
   ...
   if (node->link[_RIGHT] == NULL)
   {
     *nodePtr = node->link[_LEFT];
     if (*nodePtr)
       (*nodePtr)->balance = 0;
   }
   else
   { 
     avlTreeTables_t *tempNode = node->link[_RIGHT];
     if (tempNode->link[_LEFT] == NULL)
     {
       tempNode->link[_LEFT] = node->link[_LEFT];
     ...
     {
       avlTreeTables_t *node2 = tempNode->link[_LEFT];
       int baseDepth = depth++;
               
       pathMap[depth] = 0;
       path[depth++] = tempNode;

       while (node2->link[_LEFT] != NULL)
       {
         tempNode = node2;
         node2 = tempNode->link[_LEFT];
         pathMap[depth] = 0;
         path[depth++] = tempNode;
       }
       ...
       node2->link[_LEFT] = node->link[_LEFT];
       tempNode->link[_LEFT] = node2->link[_RIGHT];
       node2->link[_RIGHT] = node->link[_RIGHT];
                                   ...
                                   unsigned int *done)
{
  ...
  otherDirection = (direction == _LEFT) ? _RIGHT : _LEFT;
  weight = (direction == _LEFT) ? _LEFT_WEIGHT : _RIGHT_WEIGHT;
  otherWeight = (direction == _LEFT) ? _RIGHT_WEIGHT : _LEFT_WEIGHT;
  direction = (direction == _LEFT) ? _LEFT : _RIGHT;
  ...
  else
  {
    ...
    if ((tempNode->balance == 0) || 
        ((direction == _RIGHT) && (tempNode == NULL))) 
    {
...
void ** avlFindEntry(avlTree_t *avl_tree, void *item)
{
   ...
   node = node2 = baseNode->link[_LEFT];

   /* empty tree case */
   if (node == NULL)
   {
     avl_tree->count++;
     newNode = baseNode->link[_LEFT] = avlNewNewNode(avl_tree);
     newNode->data = item;
     newNode->link[_LEFT] = NULL;
     newNode->link[_RIGHT] = NULL;
     newNode->balance = _LEFT;
     return((void *)(&newNode->data));
   }

   /* find match and return or create new node and break */
   for (depth = 0; depth < _AVL_MAX_DEPTH; depth++)  
   {
     ...
     if (diff == AVL_LESS_THAN)
     {
       ...
       newNode = node2->link[_LEFT];
       if (newNode == NULL)
       {
         newNode = avlNewNewNode (avl_tree);
         node2->link[_LEFT] = newNode;
         break;
       }
     }
     ...
     else if (diff == AVL_GREATER_THAN)
     {
       ...
       newNode = node2->link[_RIGHT];
       if (newNode == NULL)
       {
         newNode = avlNewNewNode (avl_tree);
         node2->link[_RIGHT] = newNode;
         break;
       }
     }
     /* Found it */
              ...
              avlTree_t *avl_tree, int direction)
{
  ...
  int weight =  (direction == _LEFT) ? _LEFT_WEIGHT: _RIGHT_WEIGHT;
  ...
  if (((weight == _LEFT_WEIGHT) && (node->balance >= 0)) ||
      ((weight == _RIGHT_WEIGHT) && (node->balance <= 0)))
  {
    node->balance += weight;
    return;
  }
  ...
  if (baseNode != &avl_tree->root && node == baseNode->link[_RIGHT]) 
    baseNode->link[_RIGHT] = shiftNode;
  else
    baseNode->link[_LEFT] = shiftNode;
...
{
  int otherDirection = (direction == _LEFT) ? _RIGHT: _LEFT;
                  ...
                  avlTreeTables_t *node2)
{
  ...
  int direction = (otherDirection == _LEFT) ? _RIGHT: _LEFT;
